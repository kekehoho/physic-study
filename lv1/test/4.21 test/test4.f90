program main
  ...
  implicit none
    ...
    real(8),dimension(12,100) :: num,mu_total
    real(8),dimension(12,100) :: q,u
    real(8),dimension(12,100) :: q_norm, u_norm
    real(8),dimension(12,100) :: mu_ave
    ...
    integer :: times
    real(8) :: times_,z_total_pre
    real(8),dimension(12) :: counter
    integer :: access
   ...
   ! t_max = 2 ! test to be fixed 
    iter_max = 10000 !should be fixed 
    pi = 4.0d0*atan(1.0d0)
    ...
    do iter = 1, iter_max
      ...
      ! stokes_pre(2) = 0.0d0 !nonpolarization
       ...
       do times = 1,12
          times_ = real(times)
          if(abs(z_total) .ge. times_*0.5d0)then
             counter(times) = counter(times) + 1.0d0
          end if
          
          if(abs(z_total) .ge. times_*0.5d0 .and. abs(z_total_pre) .lt. times_*0.5d0 .and. counter(times) .lt. 2.0d0)then
             do i = 1,100
                i_ = real(i)
 ...
 do j = 1,12
    ...
    write(fileName, '("test_",i2,"_layer.csv")') j
