program main
  ...
  implicit none
    ...
    integer :: t,t_max, i,j,number, m, file_i
    ...
    real(8) :: mu_pre
    real(8),dimension(100) :: q_pre,u_pre,n_pre,q_pre_ave,u_pre_ave
    real(8),dimension(100) :: mu_pre_total,mu_pre_ave
   ...
   ! t_max = 2 ! test to be fixed 
    ...
    pi = 4.0d0*atan(1.0d0)
    depth = 2.0d0 !*10.0d0**(163.0d0/40.0d0) ! shuold be fixed
    tau_slab = 2.0d0
    ...
    write(fileName1, '("test_mu_pre_depth2_q_distribution_nonpol.csv")')
    write(fileName2, '("test_mu_pre_depth2.csv")')
    ...
    write(1, '("q_pre, N_pre")')
    write(2, '("mu_pre, q_pre, u_pre, N_pre")')
...
! print'(f7.2,f7.2,f7.3,f7.3)',abs(mu),abs(mu_pre),stokes_pre(1),stokes_post(1)   
 ...
 !   end do
   ...
   ! if(abs(mu) .ge. 0.6d0 .and. abs(mu) .le. 0.8d0)then
          do i = 1,100
          i_ = real(i)
          if(stokes_pre(1) .ge. (i_-1.0d0)*0.02d0 -1.0d0 .and. stokes_pre(1) .lt. i_*0.02d0 - 1.0d0)then
          q_pre(i) = q_pre(i) + stokes_pre(1)
         ! u_pre(i) = u_pre(i) + stokes_pre(2)
          n_pre(i) = n_pre(i) + 1.0d0
       end if

          if(abs(mu_pre) .ge. (i_- 1.0d0)*0.01d0 .and. abs(mu_pre) .lt. i_*0.01d0)then
          q(i) = q(i) + stokes_pre(1)
          u(i) = u(i) + stokes_pre(2)
          num(i) = num(i) + 1.0d0
          mu_total(i) = mu_total(i) + abs(mu_pre)
       end if
    end do
   ! q_pre_ave = q_pre/n_pre
   ! u_pre_ave = u_pre/n_pre
   ! mu_pre_ave = mu_pre_total/n_pre
 !end if
...
do j = 1,100
    q_pre_ave(j) = q_pre(j)/n_pre(j)
   ! u_pre_ave(j) = u_pre(j)/n_pre(j)
    write(1, '(f15.6,",",f15.0)')&
         q_pre_ave(j),n_pre(j)
    ...
    write(2, '(f15.6,",",f15.6,",",f15.6,",",f15.0)')&
