  ...
  implicit none
   ...
   ! t_max = 2 ! test to be fixed 
    ...
    alpha_0 = (tau_slab/(depth*(1.0d0-exp(-1.0d0))))!*10.0d0**(-133.0d0/40.0d0)
     ...
     write(fileName, '("test_mu_pre_depth_2_q.csv")')
    ...
    write(fileNum, '("mu_pre, q_pre, u_pre, N_pre")')
...
! print'(f7.2,f7.2,f7.3,f7.3)',abs(mu),abs(mu_pre),stokes_pre(1),stokes_post(1)   
 ...
 !   end do
   ...
   ! if(abs(mu) .ge. 0.6d0 .and. abs(mu) .le. 0.8d0)then
          ...
          mu_pre_total(i) = mu_pre_total(i) + abs(mu_pre)
       end if
    end do
...
do j = 1,100
    ...
    mu_pre_ave(j) = mu_pre_total(j)/n_pre(j)
    write(fileNum, '(f15.6,",",f15.6,",",f15.6,",",f15.0)')&
         mu_pre_ave(j) ,q_pre_ave(j),u_pre_ave(j),n_pre(j)
 end do
