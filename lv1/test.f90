program main
  ...
  implicit none
    ...
    real(8) :: x_delta ! shuoud be fixed
    real(8) :: radius,r_plus
    ...
    t_max = 1000000 !!!3/10
    iter_max = 10000000 !3/10
    pi = 4.0d0*atan(1.0d0)
    depth = 6.0d0
    tent_section = 0.6d0


     write(fileName, '("test.csv")')
    ...
    do iter = 1, iter_max
         ...
         !    call lorentz_trans(k_f_pre, k_e_pre, beta, gamma, n, k0_e_pre) ! lorentz
          ...
          do m = 1,1000
             ...
             if(abs(z_total) .ge. 0.0d0 .and. abs(z_total) .lt. 1.0d0)then !shuoud be fixed
                conf = alpha(1)
             else if(abs(z_total) .ge. 2.0d0 .and. abs(z_total) .lt. 2.0d0)then !shuoud be fixed 
                conf = alpha(2)
             else if(abs(z_total) .ge. 4.0d0 .and. abs(z_total) .lt. 3.0d0)then !shuoud be fixed 
                conf = alpha(3)
             else if(abs(z_total) .ge. 6.0d0 .and. abs(z_total) .lt. 4.0d0)then !shuoud be fixed 
                conf = alpha(4)
             else if(abs(z_total) .ge. 8.0d0 .and. abs(z_total) .lt. 5.0d0)then !shuoud be fixed 
                conf = alpha(5)
             else
                conf = alpha(6)
             end if
     ...
     ! if(z_total .lt. 0) exit
             ...
             k_f_post(4) = k_e_post(4)
                   ...
                   if (cross_section_norm .gt. r_plus .and. abs(z_total + k_f_post(4) ) .gt. depth) exit !rejection                                                                                                    
             end do
