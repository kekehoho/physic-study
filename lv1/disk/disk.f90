program main
  ...
  implicit none
   ...
   ! t_max = 2 ! test to be fixed 
    ...
    pi = 4.0d0*atan(1.0d0)
    depth = 3.0d0
    !depth = 3.0d0*10.0d0**(41.0d0/20.0d0) !middle
    !depth = 3.0d0*10.0d0**(163.0d0/40.0d0) ! outer
    alpha_0 = 1.0d0 !inner
    !alpha_0 = 2.0d0*10.0d0**(-33.0d0/20.0d0)!middle
    ! alpha_0 = 1.0d0*10.0d0**(-133.0d0/40.0d0)!outer 
    tau_slab = depth*alpha_0*(1-exp(-1.0))
    tent_section = depth/20.0d0
    ! alpha_0 = (tau_slab/(depth*(1.0d0-exp(-1.0d0))))!*10.0d0**(-133.0d0/40.0d0)
    ! alpha_0 = (1.0d0/(depth*(1.0d0-exp(-1.0d0))))*1.0d0*10.0d0**(-133.0d0/40.0d0)

     write(fileName, '("lv1_test_alpha_exp_inner.csv")')
 ...
 print'(f10.1,f10.1)',t_ave,tau_slab
 ...
 do j = 1,100
    ...
    write(fileNum, '(f15.6,",",f15.6,",",f15.6,",",f15.0)')&
         i_*0.01d0 ,q_norm(j),u_norm(j),num(j)
 end do
