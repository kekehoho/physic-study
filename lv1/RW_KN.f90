program main
  ...
  implicit none
    ...
    real(8) :: radius,radius_first
    real(8) :: theta_first,phi_first
    ...
    t_max = 100000000
   ! t_max = 0 ! test to be fixed 
    iter_max = 100000000 !should be fixed 
    pi = 4.0d0*atan(1.0d0)
    depth = 6.0d0 ! shuold be fixed
    tent_section = 0.1d0
    radius = 2.0d0/3.0d0


     write(fileName, '("spheroidal_angel.csv")')
...
!    alpha(1:6) =(/1.0d0, 1.0d0, 0.25d0, 0.25d0, 0.25d0, 0.25d0/)
   ...
   do i = 1,6 ! shuoud be fixed
    ...
    do iter = 1, iter_max
      ...
      ! z_total = 0.0d0
       ...
       theta_first = pi*grnd()
       phi_first = 2.0d0*pi*grnd()
       radius_first = grnd()
       
       x_total = a*radius_first*sin(theta_first)*cos(phi_first)
       y_total = a*radius_first*sin(theta_first)*sin(phi_first)
       z_total = b*radius_first*cos(theta_first)
       ...
       do t = 0, t_max
            ...
            ! end if
             ...
             tau = tent_section*conf
              ...
              if(exp(-tau) .lt. random)exit
                ...
                x = k_f_post(2)*tent_section/k_f_post(1)! shuoud be fixed
                y = k_f_post(3)*tent_section/k_f_post(1)! shuoud be fixed
             ...
             !  if(abs(z_total) .gt. depth)exit!slab
               if((x_total**2.0d0+y_total**2.0d0)/9.0d0 + z_total**2.0d0 .gt. radius**2.0d0)exit
          end do
        ...
        ! delta_tau = log(1/(1-random))
         ...
         s_p = section + delta_section
          ...
          x_delta = k_f_post(2)*delta_section/k_f_post(1)! shuoud be fixed
          y_delta = k_f_post(3)*delta_section/k_f_post(1)! shuoud be fixed 
          z_delta = k_f_post(4)*delta_section/k_f_post(1)! shuoud be fixed
          

       z_total = z_total + z_delta ! shuoud be fixed
       y_total = y_total + y_delta
       x_total = x_total + x_delta ! shuoud be fixed
