program main
  ...
  implicit none
    ...
    t_max = 1000000 !!!3/10
    iter_max = 100000000 !3/10
    pi = 4.0d0*atan(1.0d0)
    depth = 6.0d0
    tent_section = 0.6d0


     write(fileName, '("Baumkuchen.csv")')
    ...
    alpha(1:6) =(/2.0d0, 2.0d0, 2.0d0, 0.5d0, 0.5d0, 0.5d0/)
       ...
       do t = 0, t_max
         ...
         !    call lorentz_trans(k_f_pre, k_e_pre, beta, gamma, n, k0_e_pre) ! lorentz
          ...
          do m = 1,1000
              ...
              radius_xy = sqrt(x_total*x_total+y_total*y_total)!shuold be fixed
             
             if(abs(radius_xy) .ge. 0.0d0 .and. abs(radius_xy) .lt. 2.0d0)then !shuoud be fixed
                conf = alpha(1)
             else if(abs(radius_xy) .ge. 2.0d0 .and. abs(radius_xy) .lt. 4.0d0)then !shuoud be fixed 
                conf = alpha(2)
             else if(abs(radius_xy) .ge. 4.0d0 .and. abs(radius_xy) .lt. 6.0d0)then !shuoud be fixed 
                conf = alpha(3)
             else if(abs(radius_xy) .ge. 6.0d0 .and. abs(radius_xy) .lt. 8.0d0)then !shuoud be fixed 
                conf = alpha(4)
             else if(abs(radius_xy) .ge. 8.0d0 .and. abs(radius_xy) .lt. 10.0d0)then !shuoud be fixed 
                conf = alpha(5)
             else
                conf = alpha(6)
             end if
             ...
             tau = tent_section*conf
              ...
              if(exp(-tau) .lt. random)exit
                ...
                y_total = y_total + y ! shuoud be fixed
