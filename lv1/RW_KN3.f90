program main
  ...
  implicit none
   ...
   ! t_max = 0 ! test to be fixed 
    iter_max = 1000000 !should be fixed 
    pi = 4.0d0*atan(1.0d0)
    depth = 6.0d0 ! shuold be fixed
    tent_section = 0.6d0


     write(fileName, '("test_individual_0.5.csv")')
    ...
    write(fileNum, '("cos(i), q, u, scattering number")')
  ...
  do i = 1,6 ! shuoud be fixed
    ...
    do iter = 1, iter_max
     ...
     write(fileNum, '(f15.6,",",f15.6,",",f15.6,",",i7)')&
        abs(mu),stokes_post(1),stokes_post(2),t
