...
contains
  ...
  subroutine compton_polarized(k_f_pre, beta,  gamma, theta, n, stokes_pre, k_f_post, stokes_post)
    use mtmod
    ...
    real(8), intent(in) :: beta, gamma, theta
    ...
    real(8) :: r
    real(8) :: n_k, n_k_post
    real(8) :: k0_e_post_dummy  ! See TODOs in calc_thomson_diff and calc_kn_diff
...
!   - mu_k     : scattering angle between pre- and post-photons
  ...
  subroutine lorentz_trans(k_f_pre, k_e_pre, beta, gamma, n, k0_e_pre)
   ...
   ! k_e_pre(1) = gamma*(k_f_pre(1) - beta*k_f_pre(4))
   ! k_e_pre(4) = gamma*(-beta*k_f_pre(1) + k_f_pre(4))
   ! k_e_pre(3) = k_f_pre(3)
   ! k_e_pre(2) = k_f_pre(2)  
  ...
  subroutine sample_scattered_photons(k0_e_pre, k_e_pre, k_e_post, mu_k, theta_post, theta)

    use mtmod

    implicit none

    real(8), intent(in)  ::  k0_e_pre, theta
    real(8), dimension(4), intent(in) :: k_e_pre
    real(8), dimension(4), intent(out) :: k_e_post
    real(8), intent(out) :: mu_k

    real(8) :: r, phi, k0_e_post,r_
    real(8) :: theta_pre, phi_pre, phi_post
    real(8), intent(out) :: theta_post
    ...
    theta_pre = acos(k_e_pre(4)/k_e_pre(1))
   ...
   !k0_e_post = k0_e_pre/( 1.0d0 + k0_e_pre*( 1.0d0 - mu_k ) )
    ...
    k_e_post(3) = k0_e_post*(-sin(theta_post)*cos(phi_post)*cos(theta_pre)*sin(phi_pre)&
         +cos(theta_post)*sin(theta_pre)*sin(phi_pre) - sin(theta_post)*sin(phi_post)*cos(phi_pre))
    ! ->
...
!   - mu_k     : scattering angle between pre- and post-photons
! output
!   - cos_chi_pre, sin_chi_pre   : Nagirner1993 (E2), Davis2009 (A5)
!   - cos_chi_post, sin_chi_post :              (E3),           (A6)
!===============================================================================
!-------------------------------------------------------------------------------
! TODO
!   - Is mu_k really need?
...
!-------------------------------------------------------------------------------
  subroutine calc_chi(k_e_pre, k_e_post, mu_k, &
                       chi_pre, chi_post, theta_post)

    implicit none

    real(8), dimension(4), intent(in)  :: k_e_pre, k_e_post
    real(8), intent(in) :: mu_k, theta_post
    ...
    do i = 1,4
       ...
       unit_k_e_post(i) = k_e_post(i)/k_e_post(1)
    end do
    ...
    sin_chi_pre = (unit_k_e_pre(3)*unit_k_e_post(2)-unit_k_e_pre(2)*unit_k_e_post(3))/denominator
...
!   - cos_chi : rotation angle
!   - sin_chi : rotation angle
! output
...
!-------------------------------------------------------------------------------
! TODO
!   - Is k0_e_post really need?
!     The energy of scattered photon was calculated in sample_scattered_photons.
!-------------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------
  ...
  subroutine lorentz_inverse(k_f_post, k_e_post, beta, gamma, n)
   ...
   ! k_f_post(1) = gamma*(k_e_post(1) + beta*k_e_post(4))
   ! k_f_post(4) = gamma*(beta*k_e_post(1) + k_e_post(4))
