program main
  ...
  implicit none
   ...
   ! real(16),dimension(100) :: mu_ave !bug? deback option check !FFLAGS = -O2 -g  -fbounds-check -fbacktrace -ffpe-trap=invalid,zero,overflow
    ...
    t_max = 1 !!!3/10
    iter_max = 100000 !3/10
    pi = 4.0d0*atan(1.0d0)
    depth = 6.0d0


    write(fileName, '("distribution_isotropical.csv")')
    ...
    write(fileNum, '("phi, theta, phi_scatter, theta_scatter")')
    ...
    do iter = 1, iter_max
      ...
      ! stokes_pre(2) = 0.0d0 !nonpolarization
       ...
       if(k_f_post(3) .gt. 0)then
          phi = acos(k_f_post(2)/(sqrt(k_f_post(2)*k_f_post(2) + k_f_post(3)*k_f_post(3))))
       else
          phi = 2.0d0*pi - acos(k_f_post(2)/(sqrt(k_f_post(2)*k_f_post(2) + k_f_post(3)*k_f_post(3))))
       end if 
    ...
    write(fileNum, '(f15.6,",",f15.6,",",f11.6,",",f11.6)')&
                  phi*180d0/pi,acos(mu)*180d0/pi,phi_post*180d0/pi,theta_post*180d0/pi
    

    count = real(iter)
    do number = 1,10
       number_ = real(number)
       if (count .eq. number_*(10**4.0d0))then
          print '(i9)',iter
       end if
    end do
