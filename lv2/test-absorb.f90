...
! integer(8) :: exec_t0, exec_t1, t_rate, count_max, t_diff  ! for exec time
    ...
    real(8),dimension(100) :: n_s,n_s_total,n_s_all
    ...
    real(8) :: delta_tau, delta_section,max_a,min_a,true_a,tau_slab,tau_slab_a
 ...
 !  call system_clock(exec_t0, t_rate, count_max)
    ...
    write(fileName, '("test-absorb_tau=5_w=10^-3(N=10^9).csv")')
...
! print'(f100.90,i5)',w,myrank
 ...
 call MPI_REDUCE(n_s, n_s_total, 100, MPI_DOUBLE_PRECISION,MPI_SUM,0, wcomm, ierr)
 ...
 do j = 1,100
    ...
    i_ = real(j)
    write(fileNum, '(f5.2,",",f20.10,",",f20.10,",",f15.0,",",f20.10,",",f10.4)')&
         i_*0.01d0 ,q_norm(j),u_norm(j),n_total(j),w_total(j),n_s_total(j)/n_total(j)
