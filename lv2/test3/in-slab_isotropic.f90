...
! integer(8) :: exec_t0, exec_t1, t_rate, count_max, t_diff  ! for exec time
    ...
    real(8),dimension(11,100) :: num,mu_total
    real(8),dimension(11,100) :: q,u,n,n_total
    real(8),dimension(11,100) :: q_total,u_total,w_total
    real(8),dimension(11,100) :: q_norm, u_norm
    real(8),dimension(11,100) :: n_s,n_s_total,n_s_all
    ...
    real(8) :: delta_tau, delta_section,tau_slab,tau_slab_a
    real(8) :: x_p,y_p,z_p,s_p
    real(8) :: z_delta,tent_section
    real(8) :: alpha_es,alpha_ff,alpha_0,alpha_a
 ...
 !  call system_clock(exec_t0, t_rate, count_max)
    ...
    write(fileName, '("test3(layer,iso,tau=10)"i2".csv")')i-1
    ...
    write(fileNum, '("mu, q, u, number, w_total, t_ave")')
    ...
    t_max = 100000000
    pi = 4.0d0*atan(1.0d0)
    rho = 1.07*10.0d0**(-6.0d0)
    kappa_es = 0.03974
    kappa_ff = 0.0002856
    alpha_0 = kappa_es*rho!scatter conficient !outer
  ! alpha_a = kappa_ff*rho!absorb conficient
    alpha_a = 0.0
   ! depth = 34464290.0d0 !outer
    ...
    ! tau_slab = sqrt(pi/2)*alpha_0*depth*erf(1.0/sqrt(2.0))
     ...
     depth = tau_slab/(alpha_0) !! kokotigaukigasuru

     ! tau_slab_a = alpha_a*depth*sqrt(pi)*erf(1.0d0)/2.0d0
     tau_slab_a = alpha_a*depth
     tent_section = depth/100 
...
!    if(myrank==0)then
!       print'("tau_es="f11.5",tau_ff="f11.5",depth="f20.5",N_per_core="i10)',tau_slab,tau_slab_a,depth,iter_max
    !    end if
    ...
    do i = 1,11
       ...
       stokes_pre(1) = 2.0d0*s-1.0d0
       if(pm .ge.0.5d0)then
          stokes_pre(2) = sqrt(1.0d0-stokes_pre(1)**2.0d0)
       else
          stokes_pre(2) = sqrt(1.0d0-stokes_pre(1)**2.0d0)
       end if
      ...
      ! stokes_pre(2) = 0.0d0 !nonpolarization
       ...
       do t = 0, t_max
            ...
            ! k_f_pre(4) = sqrt(k_f_pre(1)**2.0d0-k_f_pre(2)**2.0d0)     
             ...
             do
               ...
               ! end if
                ...
                if (cross_section_norm .gt. 0) exit !rejection
          ...
          do 
              ...
              alpha_es = alpha_0!*exp(-abs(z_total)**2.0d0/(2.0d0*depth**2.0d0))

              alpha_ff = alpha_a!*exp(-abs(z_total)**2.0d0/(depth**2.0d0))

             tau = tent_section*alpha_es

              if(exp(-tau) .lt. random)exit
             
              section = section + tent_section

              tau_a = tent_section*alpha_ff! to be fixed
         ...
         delta_section = delta_tau/alpha_es

         s_p = section + delta_section

         tau_a = delta_section*alpha_ff ! to be fixed
 ...
 !        print'(f100.90,i5)',w,myrank
       ...
       do j = 1,11
          j_=real(j)
          if(abs(z_pre_total) .le. depth*(j_-1.0d0)*0.1d0 .and. abs(z_total) .ge. depth*(j_-1.0d0)*0.1d0 &
             .and. counter(j) .eq. 0.0d0)then
             counter(j)=counter(j)+1.0d0
             do i = 1,100
                i_ = real(i)
                if(abs(mu) .ge. (i_- 1.0d0)*0.01d0 .and. abs(mu) .lt. i_*0.01d0)then
                   q(j,i) = q(j,i) + stokes_post(1)*w
                   u(j,i) = u(j,i) + stokes_post(2)*w
                   n_s(j,i) = n_s(j,i) + t*w
                   n(j,i) = n(j,i) + 1.0d0
                   num(j,i) = num(j,i) + w
                end if
...
! print'(f100.90,i5)',w,myrank
do j=1,11
 call MPI_REDUCE(q(j,:), q_total(j,:), 100, MPI_DOUBLE_PRECISION,MPI_SUM,0, wcomm, ierr)
 call MPI_REDUCE(u(j,:), u_total(j,:), 100, MPI_DOUBLE_PRECISION,MPI_SUM,0, wcomm, ierr)
 call MPI_REDUCE(num(j,:), w_total(j,:), 100, MPI_DOUBLE_PRECISION,MPI_SUM,0, wcomm, ierr)
 call MPI_REDUCE(n(j,:), n_total(j,:), 100, MPI_DOUBLE_PRECISION,MPI_SUM,0, wcomm, ierr)
 call MPI_REDUCE(n_s(j,:), n_s_total(j,:), 100, MPI_DOUBLE_PRECISION,MPI_SUM,0, wcomm, ierr)
 
 if (myrank ==0) then
 do i = 1,100
    q_norm(j,i) = q_total(j,i)/n_total(j,i)
    u_norm(j,i) = u_total(j,i)/n_total(j,i)
    i_ = real(i)
    write(j, '(f5.2,",",f20.10,",",f20.10,",",f15.0,",",f20.10,",",f10.4)')&
         i_*0.01d0 ,q_norm(j,i),u_norm(j,i),n_total(j,i),w_total(j,i),n_s_total(j,i)/n_total(j,i)
