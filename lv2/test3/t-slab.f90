...
! integer(8) :: exec_t0, exec_t1, t_rate, count_max, t_diff  ! for exec time
    ...
    real(8),dimension(10) :: num,mu_total
    real(8),dimension(10) :: q,u,n,n_total
    real(8),dimension(10) :: q_total,u_total,w_total
    real(8),dimension(10) :: q_norm, u_norm, mu_norm
    real(8),dimension(10) :: all_mu
    real(8),dimension(10) :: total_section,all_section
    ...
    !test
    real(8),dimension(10) :: scatter_times,scatter_times_all
    !test
    integer :: rank_number
    integer :: access
 ...
 !  call system_clock(exec_t0, t_rate, count_max)
    ...
    write(fileNum, '("depth, q, u, section, mu_ave,scatter_times")')
    ...
    alpha_0 = 1.0d0!scatter conficient !outer 
  ...
  ! iter_max = 100
   ...
   ! tau_slab = sqrt(pi/2)*alpha_0*depth*erf(1.0/sqrt(2.0))
    ...
    do i = 1,10
          ...
          u_total(i) = 0.0d0
          mu_total(i) = 0.0d0
          total_section(i) = 0.0d0
          all_section(i) = 0.0d0
          scatter_times(i) = 0.0d0
          scatter_times_all(i) = 0.0d0
    end do
          ...
          !    if (w .lt. 10.0d0**(-3.0d0))exit
                 ...
                 do j = 1,10
                    ...
                    if(abs(z_total) .lt. depth*j_*0.1d0 .and. abs(z_total) .ge. depth*(j_-1.0d0)*0.1d0)then
                       ...
                       u(j) = u(j) + stokes_post(2)*tau
                       total_section(j) = total_section(j) + tau
     ...
     !     if (w .lt. 10.0d0**(-3.0d0))exit
         ...
         delta_section = delta_tau/alpha_0
...
! print'(f100.90,i5)',w,myrank
 ...
 if (myrank ==0) then
    do i = 1,10
       ...
       i_ = real(i)
       write(1, '(f5.2,",",f20.10,",",f20.10,",",f30.2,",",f20.10,",",f20.10)')&
            i_*0.1d0*tau_slab ,q_norm(i),u_norm(i),all_section(i)/iter_max,mu_norm(i),scatter_times_all(i)/iter_max
    end do
end if
...
!if(myrank==0)then
!   print'("all_tau="f20.2",per_tau="f20.2)',all_section,total_section
!end if
