...
! integer(8) :: exec_t0, exec_t1, t_rate, count_max, t_diff  ! for exec time
    ...
    real(8),dimension(100) :: q_total,u_total,n_total
 ...
 !  call system_clock(exec_t0, t_rate, count_max)
    ...
    t_max = 100000000
    pi = 4.0d0*atan(1.0d0)
    !alpha_0 = 4.33*10.0d0**(-6.0d0)!outer
    alpha_0 = 1.0
   ...
   ! iter_max = 100
    ...
    if(myrank==0)then
       print'("tau="f11.5",N_per_core="i10)',tau_slab,iter_max
    end if
    ...
    do number = 1,10
       number_ = real(number)
       if (count .eq. number_*(10**5.0d0))then
...
! print'(f11.5,i5)',q(80),myrank
 ...
 call MPI_REDUCE(num, n_total, 100, MPI_DOUBLE_PRECISION,MPI_SUM,0, wcomm, ierr)
...
if (myrank ==0) then
 do j = 1,100
    ...
    write(fileNum, '(f15.6,",",f15.6,",",f15.6,",",f15.0)')&
         i_*0.01d0 ,q_norm(j),u_norm(j),n_total(j)
 end do
end if
